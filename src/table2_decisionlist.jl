...
const TREPAN_MAX_DEPTH = Dict("tictactoe" => 4, "mammographic_masses" => 4, "house-votes" => 4)
...
for dataset in datasets
    ...
    for randomseed in [153, 2025, 2, 987654321, 5555, 789987, 98529, 7, 2806, 1548]
        ...
        while n_trees >= 2
            ...
            labels_test,
            features,
            labels = learn_and_convert_decisionlist(n_trees, dataset, 3, randomseed;)
        ...
        timerefne = @elapsed begin
            nf = REFNE.refne(f, rangeXmin, rangeXmax; L=cfg.L, max_depth=cfg.max_depth, ott_mode=cfg.ott_mode, use_model_featurenames=true)
        end
        ...
        y_pred = apply(
            ...
            SoleData.scalarlogiset(
                DataFrame(features_test, ["V$(i)" for i = 1:size(features_test, 2)]);
                allow_propositional=true,
            ),
        )
        ...
        for (name, algorithm, exec_time) in algorithms
            ...
            for (rule_id, eval) in enumerate(rule_evaluations)
                ...
                natoms_expl = map(
                    expls -> let
                        x = Float64[SoleLogics.natoms(e) for e in expls]
                        length(unique(x)) == 1 ?
                ...
                num_atoms = SoleLogics.natoms(antecedent(rule))
...
open("tab2_randomdecisionlist.csv", "w") do csv_file
    println(
        csv_file,
