...
# ds = dataset nanme
# y = supervised column
# rm = columns to be igbnored (ex: ids)
datasets = [
    (ds="banknote", y="class"),
...
datatreatments = Vector{DT.DataTreatment}(undef, length(datasets))
...
Threads.@threads for i in eachindex(filepaths)
    ...
    features, target = haskey(datasets[i], :y) ?
        (select(dataset, Not(datasets[i].y)), dataset[!, datasets[i].y]) :
        (dataset, nothing)
    ...
    datatreatments[i] = DT.load_dataset(
        features,
        target;
...
datasets = Vector{SX.DataSet}(undef, length(datasets))

# work on Datatreatment objects
for i in eachindex(datatreatments)
    datasets[i] = setup_dataset(datatreatments[i])
