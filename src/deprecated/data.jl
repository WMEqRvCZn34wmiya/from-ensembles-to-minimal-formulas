function load_data_hardcoded(hard_coded_dataset; seed=-1)
    dataset_path = "../dataset/$hard_coded_dataset/$hard_coded_dataset.data"
    ...
    if hard_coded_dataset == "zoo"
        ...
        data = DelimitedFiles.readdlm(joinpath(@__DIR__, dataset_path), ',')
    ...
    elseif hard_coded_dataset == "breast_cancer"
        ...
        data = DelimitedFiles.readdlm(joinpath(@__DIR__, dataset_path), ',', String)
    ...
    elseif hard_coded_dataset == "htru2"
        # htru2:
        raw = read(joinpath(@__DIR__, dataset_path), String)
        clean = replace(raw, "\r\n" => "\n", "\r" => "\n")
        data = DelimitedFiles.readdlm(IOBuffer(clean), ',')
        #@show joinpath(@__DIR__, dataset_path)
    ...
    elseif (
        ...
        if seed == 2 || seed == 987654321
            seed = 123456
            Random.seed!(seed)
        elseif seed == 5555 || seed == 1548
            seed = 2025
            Random.seed!(seed)
        elseif seed == 7 || seed == 2806 || seed == 153
            seed = 1314
            println(seed)
            Random.seed!(seed)
        elseif seed == 789987 || seed == 98529
            seed = 251201
            Random.seed!(seed)
        end
        dataset_path = "../dataset/veichle/$hard_coded_dataset.data"
        # cryotherapy:
        data = DelimitedFiles.readdlm(joinpath(@__DIR__, dataset_path), ',')
    ...
    elseif hard_coded_dataset == "statlog"
        if seed == 2 || seed == 987654321
            seed = 12345
            Random.seed!(seed)
        elseif seed == 5555 || seed == 153 || seed == 7
            seed = 2025
            Random.seed!(seed)
        elseif seed == 789987 || seed == 98529
            seed = 1111
    ...
    elseif hard_coded_dataset == "urinary-d1" || hard_coded_dataset == "urinary-d2"
        ...
        dataset_path = "../dataset/diagnosis/diagnosis.data"

        # Leggi il file come binario per rimuovere eventuale BOM
        raw_data = read(joinpath(@__DIR__, dataset_path))
        ...
        for i = 1:num_rows
            ...
            for j = 2:6
                ...
                numerical_features[i, j] =
                    lowercase(strip(feature_value)) == "yes" ? 1.0 : 0.0
            end
        end
    ...
    elseif hard_coded_dataset == "mushroom"
        ...
        for col = 2:size(data, 2)  # Inizia dalla colonna 2 (la prima è l'etichetta)
            ...
            if haskey(mappings, col)
                ...
                col_mapping =
                    Dict(val => Float64(i - 1) for (i, val) in enumerate(unique_vals))
    ...
    elseif hard_coded_dataset == "monks-1"
        Random.seed!(1010)
        dataset_path = "../dataset/monks/$hard_coded_dataset.data"
        # Monks: 7 columns (1 class, 6 features, 1 extra to ignore)
        data = DelimitedFiles.readdlm(joinpath(@__DIR__, dataset_path), ' ', skipblanks=true)
    ...
    elseif hard_coded_dataset == "tictactoe"
        ...
        for i = 1:size(data, 1)
            for j = 1:9  # first 9 columns are the grid positions
                if data[i, j] == "x"
    ...
    elseif hard_coded_dataset == "house-votes"
        ...
        for i = 1:size(data, 1)
            for j = 2:17  # skip the first column (class)
                if data[i, j] == "y"
                    features[i, j-1] = 1.0
                elseif data[i, j] == "n"
                    features[i, j-1] = 0.0
    ...
    elseif hard_coded_dataset == "primary-tumor"
        ...
        for i = 1:size(data, 1)
            for j = 2:18  # skip the first column (class)
                if data[i, j] == "?"
...
function load_data_hc(hard_coded_dataset)
    ...
    dataset_path = "$hard_coded_dataset/$hard_coded_dataset.data"
    ...
    elseif hard_coded_dataset == "monks"
        data = DelimitedFiles.readdlm(datasejoinpath(@__DIR__, dataset_path)t_path, ' ', skipblanks=true)
